...
import { useGtagConsent } from "./use-gtag-consent";
...
describe("useGtagConsent", () => {
  ...
  it("should load and accept all consent", async () => {
    const { acceptAll } = useGtagConsent({ tagId });
  ...
  it("should reject all", () => {
    const { rejectAll } = useGtagConsent({
      tagId,
    });
  ...
  it("should not have consent", async () => {
    const { hasConsent } = useGtagConsent({ tagId });
